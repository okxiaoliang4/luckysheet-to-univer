...
function createSiteStatic() {
  ...
  try {
    ...
    console.warn(
      '\x1B[33m%s\x1B[0m',
      `Warning: Unable to read ${licenseFilePath}. Work on Free Mode, if you want to use the Business Mode, you can get a 30-day free trial license from https://univer.ai/pro/license`,
    )
  }
  ...
  filesToReplace.forEach((file) => {
    ...
    if (fs.existsSync(filePath)) {
      ...
      content = content.replace(
        /%%UNIVER_CLIENT_LICENSE_PLACEHOLDER%%/g,
        licenseContent,
      )
...
const server = app.listen(process.env.CLIENT_PORT || 3010, () => {
  console.log(
    '\x1B[36m%s\x1B[0m',
    `Univer Demo UI running on http://localhost:${server.address().port}`,
  )
  console.log(
    '\x1B[36m%s\x1B[0m',
  ...
  console.log(
    '\x1B[32m%s\x1B[0m',
    'If you want to integrate the Univer frontend SDK, please read: https://univer.ai/guides/sheet/getting-started/quickstart',
  )
  console.log(
    '\x1B[35m%s\x1B[0m',
    'For more information about the Univer server, please read: https://univer.ai/guides/sheet/server/docker',
  )
})
